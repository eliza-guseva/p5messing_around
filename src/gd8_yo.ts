...
function sizeCanvas(img: p5.Image, p: p5){
    // work for both portrait and landscape images
...
function respawnParticle(
    p: p5, img: p5.Image, 
    memo: Map<[number, number], Boolean>
    ): [number, number]{
...
var sketch = (p: p5) => {
    let img: p5.Image;
    p.preload = () =>
        img = p.loadImage('public/bare_tree2.jpg'
        );
    let particleNum = 800;
    let particles: [number, number][] = [];
    let memo = new Map<[number, number], Boolean>();
    ...
    p.draw  = () => {
        ...
        for (let i=0; i<particles.length; i++){
            ...
            p.circle(x, y, 1);
            let windX = p.noise(
                x * 0.1, y * 0.1, 1000 + 0.01 * p.cos(p.frameCount)
                ) * 2 - 1.0 - 0.2 * (x > p.width / 2 ? -1 : 1);
            //windX = windX * windX;
            let windY = p.noise(
                x * 0.001, y * 0.001,  0.01 * p.sin(p.frameCount)
                ) * 2 - 1.7;
            //windY = windY * windY;
            particles[i][0] += windX;
            particles[i][1] += windY;
            if (
                ...
                [particles[i][0], particles[i][1]] = respawnParticle(
                    p, img, memo
                    );
