...
function fillCell(cells, a_col, a_row) {
    cells[a_col][a_row] = 1;
}
function flipCell(cells, a_col, a_row) {
    cells[a_col][a_row] = 1 - cells[a_col][a_row];
}
function drawCells(p, cells) {
...
function countNeighbors(cells, a_col, a_row) {
    let numN = 0;
    for (let i = -1; i < 2; i++) {
        for (let j = -1; j < 2; j++) {
...
new p5(sketch);
//# sourceMappingURL=gd9_1_cell.js.map