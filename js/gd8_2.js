...
function distributeOnLowerBorder(p, particles, i) {
    particles[i][0] = p.random() * p.width;
...
var sketch = (p) => {
    ...
    p.draw = () => {
        ...
        for (let i = 0; i < particles.length; i++) {
            let x = particles[i][0];
            let y = particles[i][1];
            let windX = p.noise(x * 0.02, y * 0.02, 0) * 2 - 1;
            let windY = p.noise(x * 0.02, y * 0.02, 1000) * 2 - 2;
            ...
            particles[i][1] += windY;
            if (particles[i][0] < -7)
                particles[i][0] += p.width + 6;
            if (particles[i][1] < -7)
                particles[i][1] += p.height + 6;
            if (particles[i][0] > p.width + 7)
                particles[i][0] -= p.width + 6;
            if (particles[i][1] > p.height + 7)
                particles[i][1] -= p.height + 6;
